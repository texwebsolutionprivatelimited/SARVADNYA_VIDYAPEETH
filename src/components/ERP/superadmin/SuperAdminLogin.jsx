...
export default function SuperAdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMsg("Authentication Successful! Redirecting to Super Admin Dashboard...");
      setTimeout(() => {
        navigate("/superadmin-dashboard");
      }, 1500);
    } catch (err) {
      console.error("SuperAdmin login error:", err);
      setError("Invalid Admin ID or Password. Try demo: ADMIN001 / superadmin@123");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between relative overflow-hidden font-sans">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-red-200/30 rounded-full blur-[140px] pointer-events-none animate-pulse" />
      <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-rose-200/30 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute top-1/3 left-0 w-[400px] h-[400px] bg-slate-200/40 rounded-full blur-[120px] pointer-events-none" />
      ...
      <header className="relative z-10 w-full border-b border-slate-200/80 bg-white/80 backdrop-blur-md px-4 sm:px-8 py-3.5 flex items-center justify-between shadow-sm">
        <Link to="/" className="flex items-center gap-3 group">
          <img
            ...
            className="w-10 h-10 object-contain rounded-full bg-white p-0.5 shadow-md shadow-red-500/10 border border-red-100 group-hover:scale-105 transition-transform"
          />
          <div className="flex flex-col">
            ...
            <span className="text-red-700 text-[10px] sm:text-[11px] font-bold tracking-widest uppercase">
              Super Admin Portal
            </span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            to="/"
            className="inline-flex items-center gap-1 px-3.5 py-1.5 rounded-full text-xs font-bold text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 transition-all"
          >
      ...
      <main className="relative z-10 flex-grow flex items-center justify-center p-4 sm:p-6 lg:p-8">
        <motion.div
          ...
          className="w-full max-w-md bg-white/95 backdrop-blur-xl rounded-3xl border border-red-100 p-6 sm:p-8 shadow-xl shadow-red-900/5 relative overflow-hidden"
        >
          {/* Header Logo */}
          <div className="text-center mb-6 pb-4 border-b border-slate-100">
            <div className="inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-red-600 to-rose-700 shadow-lg shadow-red-300/40 mb-3">
            ...
            <p className="text-xs text-red-700 font-bold uppercase tracking-wider mt-1">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={loading}
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-600 focus:bg-white transition-all"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  disabled={loading}
                  className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-600 focus:bg-white transition-all"
                />
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between pt-1">
              <label className="flex items-center gap-2 cursor-pointer text-xs font-medium text-slate-600 select-none">
                <input
                  ...
                  className="w-4 h-4 rounded border-slate-300 bg-white text-red-600 focus:ring-red-500"
                />
                <span>Remember Admin ID</span>
              </label>
            ...
            <div className="pt-2">
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 px-5 rounded-xl bg-gradient-to-r from-red-700 via-rose-700 to-red-800 hover:from-red-800 hover:to-rose-800 text-white font-bold text-sm shadow-lg shadow-red-600/20 border border-red-500/20 flex items-center justify-center gap-2 transition-all active:scale-[0.99] disabled:opacity-70"
              >
                {loading ? (
                  <>
          ...
          <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col gap-2 text-center">
            <Link to="/student" className="text-xs font-bold text-purple-600 hover:underline flex items-center justify-center gap-1">
              <GraduationCap className="w-3.5 h-3.5" />
              Student Login →
            </Link>
            <Link to="/staff" className="text-xs font-bold text-indigo-600 hover:underline flex items-center justify-center gap-1">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-xs text-slate-500 mb-4">
                Enter your Admin ID or registered email for password recovery.
              </p>

              {resetSent ? (
                <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-semibold flex items-center gap-2">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                  <span>Reset instructions sent! Contact the IT department.</span>
                </div>
              ) : (
                <form onSubmit={handleForgotSubmit} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. ADMIN001"
                      className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:bg-white transition-all"
                    />
                  </div>
                  <div className="flex items-center justify-end gap-2 pt-2">
                    ...
                    <button
                      type="submit"
                      className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-red-700 hover:bg-red-800 transition-colors"
                    >
                      Send Reset Request
                    </button>
                  </div>
        ...
        <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-2.5 text-[11.5px] text-slate-600">
          ...
          <p className="text-center sm:text-right text-[11px] text-slate-500">
            ...
            <a
              ...
              className="text-red-700 hover:text-red-800 font-black tracking-wide transition-colors hover:underline"
            >
